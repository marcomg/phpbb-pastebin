{include file='header.tpl'}
        ...
        <h1>{$title}</h1>
            <form class="form-inline" method="post">
                ...
                <input class="btn btn-info" type="submit" formaction="index.php?q=acp/check_for_updates" value="{t}Check for updates{/t}">
...
{section name=id loop=$tid}
                <li>
                    {t}Posting{/t} {t}from{/t} <b>{$username[id]}</b> {t}posted at{/t} <b>{$posted[id]|date_format:"%d/%m/%Y %H:%M:%S"}</b> {t}espires:{/t} <b>{if $expires[id]==-1}{t}Never{/t}{else}{{$posted[id]+$expires[id]}|date_format:"%d/%m/%Y %H:%M:%S"}{/if}</b> {t}Paste type:{/t} <b>{if $hidden[id]}{t}hidden{/t}{else}{t}public{/t}{/if}</b>
                    <br/>
...
{/section}
            <ul class="pager">
                ...
                <li class="next{if {$tid|@count} != 10} disabled{/if}">
                    <a href={if {$tid|@count} != 10}#{else}"index.php?q=acp/view/{$from+10}"{/if}>{t}Older{/t} &rarr;</a>
                </li>
            </ul>
        </div>
